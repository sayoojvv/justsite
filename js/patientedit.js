$(document).ready(function () {
    ...
    $('#btnsubmit').on('click', function () {
        $('form#patientedit').parsley().validate();
        validateFront();
    });
});
var validateFront = function () {
    console.log('parseley valid = ' + $('form#patientedit').parsley().isValid());
    if (false === $('form#patientedit').parsley().isValid()) {
...
function patientedit() {
    var formData = new FormData($("form#patientedit")[0]);
    $.ajax({
        url: 'scripts/patientedit.php',
        type: 'POST',
        data: formData,
        async: false,
        dataType: 'json',
